	...
	def readVoiceEvent(self,deltaT):
		...
		if(type >> 4 == 0x9):
			...
			if(velocity == 0):
				...
				self.log(self.deltaTime/self.division,self.key2note[key])
				self.notes.append([(self.deltaTime/self.division),self.key2note[key]])
			else:
				#Real keypress
				self.log(self.deltaTime/self.division,self.key2note[key])
				self.notes.append([(self.deltaTime/self.division),self.key2note[key]])
